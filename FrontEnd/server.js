...
const server = http.createServer((req, res) => {
    ...
    //doctor page
        ...
        fs.readFile('doctor.html', 'utf-8', (err, doctorContent) => {
            if (err) {
                res.writeHead(500, { 'Content-Type': 'text/plain' });
                res.end('Error loading doctor page');
            } else {
                res.writeHead(200, { 'Content-Type': 'text/html' });
                res.end(doctorContent);
            }
        });
    }

    //temp
    else if (req.method === 'GET' && req.url === '/img/doctor.png') {
        fs.readFile('./img/doctor.png', (err, imgContent) => {
            if (err) {
                res.writeHead(500, { 'Content-Type': 'text/plain' });
